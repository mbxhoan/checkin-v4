@php
    ...
    foreach ($sidebarMenus as $key => $menu) {
        $isRestrictedMenu = count($exceptMenus) && in_array($key, $exceptMenus, true);
        if ($isRestrictedMenu) {
        ...
        if (isset($menu['subMenus']) && is_array($menu['subMenus'])) {
            foreach ($menu['subMenus'] as $subKey => $subMenu) {
                // Mark restricted subMenus instead of hiding, so basic users can still see them.
                $isRestrictedSubMenu = $isRestrictedMenu || (count($exceptMenus) && in_array($subKey, $exceptMenus, true));
                if ($isRestrictedSubMenu) {
                    $menu['subMenus'][$subKey]['restricted'] = true;
                }
    ...
    $groups = config("sidebar-menu.admin_groups");
            ...
            <ul class="metismenu list-unstyled" id="side-menu">
                @foreach ($groups as $groupKey => $group)
                    @foreach ($sidebarMenus as $key => $menu)
                        ...
                        @endif
                    @endforeach
                    <li class="menu-title" key="t-{{ $groupKey }}">
                        {{ $translateOrFallback("sidebar.groups.{$groupKey}", $group) }}
                    </li>
                    @foreach ($sidebarMenus as $key => $menu)
                        <li class="{{ request()->route()->named($menu['route_prefix']) ? 'mm-active' : '' }}">
                            @if (isset($menu['group']) && $menu['group'] == $groupKey)
                                @php
                                    $xIconPrefix = $menu['x_icon_prefix'] ?? "fa-solid";
                                    $menuLabel = $translateOrFallback("sidebar.menus.{$key}", $menu['text'] ?? ucfirst((string) $key));
                                @endphp
                                @if (isset($menu['subMenus']))
                                    <a href="javascript: void(0);" class="has-arrow waves-effect {{ request()->route()->named($menu['route_prefix']) ? 'mm-active' : '' }}">
                                        <i class="{{ "{$xIconPrefix} fa-{$menu['x_icon_name']} fa-xs" }}" aria-hidden="true"></i>
                                        <span key="t-{{ $key }}">
                                            {{ $menuLabel }}
                                        </span>
                                        @if (!empty($menu['restricted']))
                                            <span class="badge rounded-pill bg-warning-subtle text-warning ms-2">
                                                <i class="fa-solid fa-lock"></i>
                                                {{ __('sidebar.badges.basic') }}
                                            </span>
                                        @endif
                                    </a>
                                    <ul class="sub-menu" aria-expanded="false">
                                        @foreach ($menu['subMenus'] as $subKey => $subMenu)
                                            <li class="{{ request()->route()->named($subMenu['route_prefix']) ? 'mm-active' : '' }}">
                                                @php
                                                    $isRestricted = !empty($subMenu['restricted']) || !empty($menu['restricted']);
                                                    $subMenuLabel = $translateOrFallback("sidebar.submenus.{$key}.{$subKey}", $subMenu['text'] ?? ucfirst((string) $subKey));
                                                    $subHref = $isRestricted
                                                        ? route('admin.feature-unavailable', [
                                                            'feature' => $key,
                                                            'label' => $menuLabel,
                                                            'sub' => $subMenuLabel,
                                                        ])
                                                ...
                                                @endphp
                                                <a href="{{ $subHref }}"
                                                    class="{{ $isRestricted ? 'text-muted' : '' }}"
                                                    key="t-{{ $subKey }}"
                                                >
                                                    {{ $subMenuLabel }}
                                                    @if ($isRestricted)
                                                        <i class="fa-solid fa-lock ms-1" aria-hidden="true"></i>
                                                    @endif
                                                </a>
                                            </li>
                                        @endforeach
                                ...
                                @else
                                    @php
                                        $isRestricted = !empty($menu['restricted']);
                                        $menuHref = $isRestricted
                                            ? route('admin.feature-unavailable', [
                                                'feature' => $key,
                                                'label' => $menuLabel,
                                            ])
                                    ...
                                    @endphp
                                    <a href="{{ $menuHref }}"
                                        class="waves-effect {{ request()->route()->named($menu['route_prefix']) ? 'active' : '' }} {{ $isRestricted ? 'text-muted' : '' }}"
                                    >
                                        <i class="{{ "{$xIconPrefix} fa-{$menu['x_icon_name']} fa-xs" }}" aria-hidden="true"></i>
                                        <span key="t-{{ $key }}">
                                            {{ $menuLabel }}
